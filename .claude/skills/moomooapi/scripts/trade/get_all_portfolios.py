...
# Map trdmarket_auth → reporting currency. moomoo's accinfo_query defaults to
# the account's home currency (HKD for Futu HK margin accounts), which makes
# US-only accounts look ~8x larger than reality. Override per-market so totals
# match position currency.
_MARKET_CURRENCY = {
    "US": Currency.USD,
    "HK": Currency.HKD,
    "HKCC": Currency.HKD,
...
def _pick_currency(trdmarket_auth, override=None):
    if override is not None:
        return getattr(Currency, override, None)
    markets = trdmarket_auth if isinstance(trdmarket_auth, list) else [trdmarket_auth]
    for m in markets:
        ms = format_enum(m) if not isinstance(m, str) else m
        if ms in _MARKET_CURRENCY:
            return _MARKET_CURRENCY[ms]
    return None
...
def query_portfolio(host, port, acc_id, trd_env, currency=None):
    ...
    try:
        # Funds
        accinfo_kwargs = dict(trd_env=trd_env, acc_id=acc_id)
        if currency is not None:
            accinfo_kwargs["currency"] = currency
        ret, acc_data = ctx.accinfo_query(**accinfo_kwargs)
        funds = {}
...
def main():
    ...
    parser.add_argument("--currency", choices=["USD", "HKD", "CNH", "JPY", "AUD", "CAD", "MYR", "SGD"], default=None,
                        help="Override reporting currency (default: USD for US markets, HKD for HK)")
    ...
    for acc in accounts:
        ...
        currency = _pick_currency(acc["trdmarket_auth"], override=args.currency)
        funds, positions = query_portfolio(host, port, acc_id, trd_env, currency=currency)
        results.append({
            "acc_id": acc_id,
            "trd_env": trd_env_str,
            "acc_type": acc["acc_type"],
            "trdmarket_auth": acc["trdmarket_auth"],
            "currency": format_enum(currency) if currency is not None else "default",
            "funds": funds,
            "positions": positions,
        })
