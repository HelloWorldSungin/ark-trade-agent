...
| Auth | token mode. Token stored in `~/.openclaw/openclaw.json` `gateway.auth.token` (file mode 0600). The systemd unit does **not** export `OPENCLAW_GATEWAY_TOKEN` — the gateway reads the config file directly. `openclaw config get gateway.auth.token` redacts the value (prints `__OPENCLAW_REDACTED__`); to retrieve the raw token, parse the JSON: `python3 -c 'import json; print(json.load(open("/home/ark-dev/.openclaw/openclaw.json"))["gateway"]["auth"]["token"])'`. |
...
|---|---|---|---|
| `tradingagents/dataflows/moomoo_news.py` (NEW) | Lifted from sister-project `moomoo-news-search` SKILL.md. Calls `GET https://ai-news-search.moomoo.com/news_search` (public, unauthenticated). Implements `get_news(ticker, start_date, end_date)` matching the existing news_data signature; date-filtered, formatted as markdown. **W4/cso-finding-3 (2026-05-13)**: returned body wrapped via `_wrap_untrusted()` with `TRUST BOUNDARY` notice + sentinel markers (`--- THIRD-PARTY UNTRUSTED CONTENT BEGIN/END ---`) so the analyst LLM treats article content as data, not instructions. | `9f072c8547b4d5ef1aad2da29a7890fd0211e77c0441906b833d5fcc6d24bd2c` | 2026-05-13 |
| `tradingagents/dataflows/moomoo_digest.py` (NEW) | Lifted from sister-project `moomoo-stock-digest` SKILL.md. Same `/news_search` endpoint as moomoo_news but framed for direction-judgment by the calling analyst (bullish/bearish/neutral interpretation directive). Implements `get_stock_digest(ticker)`. **W4/cso-finding-3 (2026-05-13)**: imports `_wrap_untrusted` from moomoo_news; directive stays OUTSIDE the wrap, item bodies INSIDE. | `d3b05e366ca5e8cf60ab191f9a667131552d5d99775cc508dbcac399aa12c867` | 2026-05-13 |
| `tradingagents/dataflows/moomoo_sentiment.py` (NEW) | Lifted from sister-project `moomoo-comment-sentiment` SKILL.md. Calls `GET https://ai-news-search.moomoo.com/stock_feed`; strips HTML, returns post list with sentiment-classification directive. Implements `get_community_sentiment(ticker)`. **W4/cso-finding-3 (2026-05-13)**: imports `_wrap_untrusted` from moomoo_news; directive stays OUTSIDE the wrap, post bodies INSIDE. | `34cc26e6283ee71f8dae9fd63f66a8adac249cf074f71e82665a302c9b9f254a` | 2026-05-13 |
...
| Web dashboard | `~/.config/systemd/user/hermes-dashboard.service` (user-level systemd, installed 2026-05-13). Binds **`127.0.0.1:9119`** (loopback-only), no auth on loopback. `ExecStart=/home/ark-dev/.local/bin/hermes dashboard --no-open --host 127.0.0.1 --port 9119`, `Restart=always`. Auto-starts on reboot via user-lingering (same mechanism as `openclaw-gateway.service`). Access from Mac with `ssh -L 9119:127.0.0.1:9119 ark-dev@192.168.68.83` then open `http://127.0.0.1:9119/`. `hermes dashboard --insecure` would bind LAN but intentionally NOT used — per Hermes' own warning, exposes API keys. |
...
| LOQ install path | `/opt/ark-trade-agent/scripts/run_hermes_proposal.py` (git-tracked on LOQ since 2026-05-12 sync repair; Mac edits flow via `git push` → LOQ `git pull`. `scp` still works for fast iteration mid-session). |
...
- ~~**Mac↔LOQ sync repair** (orthogonal): push current Mac project commits to GitHub, LOQ `git pull` to refresh, commit the un-tracked scripts (`init_eval_ledger.py`, `notify_discord.py`, `run_prediction_cycle.py`, `run_hermes_proposal.py`, `score_metrics.py`) so the ledger of what's deployed where becomes git-tracked.~~ **DONE 2026-05-12** — pushed `ce09c31..84583bf` (7 commits) to GitHub; LOQ fast-forwarded; 5 scripts now git-tracked byte-identically (verified via sha256sum). Future Mac edits flow `git push` → LOQ `git pull`. `scp` retained for fast mid-session iteration.
...
- **/cso pass closeouts (2026-05-13)**: chain step 14 ran. **Finding 3 (indirect prompt injection via moomoo news → Hermes)** mitigated in three places: (a) `can_apply_proposal_safely()` placeholder added to `run_hermes_proposal.py` — forward-looking gate that returns `(False, reason)` unconditionally in v0 (sample-size + held-out replay + doctrinal hard floor); any future `apply_proposal.py` MUST call this. (b) Trust-boundary sentinel markers + `TRUST BOUNDARY` notice baked into `moomoo_news.py` / `moomoo_digest.py` / `moomoo_sentiment.py` via `_wrap_untrusted()` so analyst LLMs treat article content as data, not instructions; TradingAgents image rebuilt to include the patch. (c) Trust-boundary prefix added to `build_hermes_prompt()` in `run_hermes_proposal.py:300` so Hermes itself is warned that the rationale/order_intent excerpts in its prompt are downstream-LLM-authored content with embedded third-party text. **Findings 1 & 2** (Discord.env + moomoo LocalStore.dat world-readable on LOQ) chmod'd to 0600. **Finding 4** (moomoo password historical argv exposure) — rotation deferred to user convenience.
...
| LOQ install path | `/opt/ark-trade-agent/scripts/score_metrics.py` (git-tracked on LOQ since 2026-05-12 sync repair; Mac edits flow via `git push` → LOQ `git pull`. `scp` still works for fast iteration mid-session). |
